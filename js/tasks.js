...
(function () {
  ...
  let sb, user, isAdmin = false, tasks = [], staff = [], exceptions = [], selected = new Set();
  ...
  function wire(){["task-queue","task-entity","task-priority"].forEach(id=>document.getElementById(id).addEventListener("change",()=>{selected.clear();render();}));document.getElementById("task-search").addEventListener("input",()=>{selected.clear();render();});
    document.getElementById("exception-form").addEventListener("submit",createException);document.getElementById("exception-list").addEventListener("click",e=>{const b=e.target.closest("[data-exception-action]");if(b)exceptionAction(b);});
