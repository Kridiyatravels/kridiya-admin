...
(function () {
  ...
  let sb, user, isAdmin = false, tasks = [], staff = [], exceptions = [], integrationRecovery = [], selected = new Set();
  ...
  function renderExceptions(){const target=document.getElementById("exception-list"),active=exceptions.filter(x=>!["resolved","closed"].includes(x.status));target.innerHTML=active.length?active.map(x=>{const overdue=new Date()>new Date(x.response_due_at),owner=staff.find(s=>s.user_id===x.owner_user_id);return '<article class="task-card'+(overdue?' is-overdue':'')+'"><div class="task-card-main"><div class="task-title"><b>'+esc(x.title)+'</b><span class="task-priority priority-'+esc(x.severity==='critical'?'urgent':x.severity)+'">'+esc(label(x.severity))+'</span>'+(x.escalation_level?'<span class="task-escalated">Level '+esc(x.escalation_level)+'</span>':'')+'</div><p>'+esc(x.description)+'</p><div class="task-meta"><span>'+esc(label(x.category))+'</span><span>'+esc(label(x.status))+'</span><span>'+esc(owner?(owner.full_name||owner.email):"Unassigned")+'</span><span>'+esc(overdue?"Response overdue":"Respond by "+fmt(x.response_due_at))+'</span><span>Resolve by '+esc(fmt(x.resolution_due_at))+'</span></div>'+(x.external_update?'<small>External update: '+esc(x.external_update)+'</small>':'')+'</div><div class="task-actions"><button class="btn btn-outline" data-exception-action="contained" data-id="'+esc(x.id)+'" type="button">Contain</button><button class="btn btn-outline" data-exception-action="recovering" data-id="'+esc(x.id)+'" type="button">Recover</button><button class="btn btn-outline" data-exception-action="escalate" data-id="'+esc(x.id)+'" type="button">Escalate</button><button class="btn btn-primary" data-exception-action="resolved" data-id="'+esc(x.id)+'" type="button">Resolve</button></div></article>';}).join(''):'<div class="empty-state"><p>No active operational exceptions.</p></div>';}
  async function loadExceptions(){const r=await sb.rpc("list_operational_exceptions",{p_status:null,p_limit:300});if(r.error)throw r.error;exceptions=r.data||[];renderExceptions();}
  async function createException(e){e.preventDefault();const f=e.currentTarget,r=await sb.rpc("create_operational_exception",{p_category:f.category.value,p_severity:f.severity.value,p_urgency:f.urgency.value,p_title:f.title.value,p_description:f.description.value,p_entity_type:f.entity_type.value,p_entity_id:f.entity_id.value||null,p_owner_user_id:f.owner_user_id.value||null,p_evidence:[]});if(r.error)return toast(r.error.message);f.reset();await Promise.all([load(),loadExceptions()]);toast("Operational exception opened.");}
  async function exceptionAction(button){const row=exceptions.find(x=>x.id===button.dataset.id);if(!row)return;const action=button.dataset.exceptionAction;let status=action,external=null,containment=null,recovery=null,resolution=null,root=null,preventive=null,escalate=false;if(action==="escalate"){status=row.status;escalate=true;}else if(action==="contained"){containment=prompt("Containment action (what was stopped or isolated):",row.containment||"");if(containment===null)return;external=prompt("Customer/company-safe update (optional):",row.external_update||"");if(external===null)return;}else if(action==="recovering"){recovery=prompt("Recovery action underway:",row.recovery_action||"");if(recovery===null)return;}else if(action==="resolved"){resolution=prompt("Resolution evidence (minimum 10 characters):",row.resolution||"");if(resolution===null)return;root=prompt("Root cause:",row.root_cause||"");if(root===null)return;preventive=prompt("Preventive action:",row.preventive_action||"");if(preventive===null)return;}const r=await sb.rpc("update_operational_exception",{p_exception_id:row.id,p_status:status,p_owner_user_id:row.owner_user_id,p_external_update:external,p_containment:containment,p_recovery_action:recovery,p_resolution:resolution,p_root_cause:root,p_preventive_action:preventive,p_escalate:escalate});if(r.error)return toast(r.error.message);await Promise.all([load(),loadExceptions()]);toast("Exception updated.");}
  function renderIntegrationRecovery(){const target=document.getElementById("integration-recovery-list"),active=integrationRecovery.filter(x=>!["recovered","abandoned"].includes(x.retry_state));target.innerHTML=active.length?active.map(x=>'<article class="task-card'+(x.retry_state==="dead_letter"?' is-overdue':'')+'"><div class="task-card-main"><div class="task-title"><b>'+esc(label(x.integration))+': '+esc(label(x.operation))+'</b><span class="task-priority priority-'+(x.retry_state==="dead_letter"?'urgent':'high')+'">'+esc(label(x.retry_state))+'</span></div><p>'+esc(x.last_error||"Waiting for an integration worker")+'</p><div class="task-meta"><span>Attempt '+esc(x.attempt_count)+' of '+esc(x.max_attempts)+'</span><span>'+esc(x.http_status||"No HTTP status")+'</span><span>'+esc(x.next_retry_at?'Next: '+fmt(x.next_retry_at):'Manual decision required')+'</span></div></div><div class="task-actions"><button class="btn btn-outline" data-integration-action="retry" data-id="'+esc(x.id)+'" type="button">Retry now</button><button class="btn btn-outline" data-integration-action="abandon" data-id="'+esc(x.id)+'" type="button">Abandon</button><button class="btn btn-primary" data-integration-action="recovered" data-id="'+esc(x.id)+'" type="button">Mark recovered</button></div></article>').join(''):'<div class="empty-state"><p>No integration retries or dead letters need staff action.</p></div>';}
  async function loadIntegrationRecovery(){const r=await sb.rpc("list_integration_recovery_queue",{p_retry_state:null,p_limit:300});if(r.error)throw r.error;integrationRecovery=r.data||[];renderIntegrationRecovery();}
  async function integrationAction(button){const action=button.dataset.integrationAction,note=prompt(action==="retry"?"Why is this retry safe now?":action==="abandon"?"Why should this operation be abandoned?":"Recovery evidence:","");if(note===null)return;const r=await sb.rpc("recover_integration_operation",{p_operation_id:button.dataset.id,p_action:action,p_note:note});if(r.error)return toast(r.error.message);await Promise.all([load(),loadExceptions(),loadIntegrationRecovery()]);toast(action==="retry"?"Retry queued for the integration worker.":"Integration operation closed.");}
  function render() {
  ...
  function wire(){["task-queue","task-entity","task-priority"].forEach(id=>document.getElementById(id).addEventListener("change",()=>{selected.clear();render();}));document.getElementById("task-search").addEventListener("input",()=>{selected.clear();render();});
    document.getElementById("exception-form").addEventListener("submit",createException);document.getElementById("exception-list").addEventListener("click",e=>{const b=e.target.closest("[data-exception-action]");if(b)exceptionAction(b);});document.getElementById("integration-recovery-list").addEventListener("click",e=>{const b=e.target.closest("[data-integration-action]");if(b)integrationAction(b);});
