...
(function () {
  ...
  async function boot() {
    ...
    document.getElementById("corporate-list").addEventListener("submit", function (event) {
      ...
      if (event.target.closest(".corporate-branch-form")) return saveBranch(event);
    });
  ...
  async function loadBranches(){const r=await sb.rpc("list_corporate_branches",{p_corporate_account_id:null});const branches=r.error?[]:(r.data||[]);rows.forEach(function(c){c.branches=branches.filter(function(b){return b.corporate_account_id===c.id;});});if(r.error)toast("Could not load branches: "+r.error.message,"error");}
  async function saveBranch(event){event.preventDefault();const form=event.target.closest(".corporate-branch-form");const r=await sb.rpc("save_corporate_branch",{p_corporate_account_id:form.dataset.accountId,p_branch_id:null,p_branch_name:form.branch_name.value,p_branch_code:form.branch_code.value||null,p_city:form.city.value||null,p_country:form.country.value||null,p_address:form.address.value||null,p_phone:form.phone.value||null,p_billing_email:form.billing_email.value||null,p_status:form.status.value,p_notes:form.notes.value||null});if(r.error){toast(r.error.message,"error");return;}toast("Corporate branch saved.");form.reset();await loadCompanies();}
  ...
  function renderCorporateCases() {
    ...
    target.innerHTML = deskCases.length ? '<div class="ops-list">' + deskCases.map(function (c) {
      ...
      const due = c.response_due_at ? new Date(c.response_due_at).toLocaleString("en-GB") : "Not set";
      return '<article class="ops-row"><div class="ops-row-main"><b>' + esc(c.company_name + " - " + c.subject) + '</b>' + (c.quote_id ? '<p><b>Linked quote:</b> ' + esc(c.quote_title || c.quote_id) + '</p>' : '') + '<p>' + esc(c.description) + '</p>' + (c.staff_response ? '<p><b>Response:</b> ' + esc(c.staff_response) + '</p>' : '') + '<div class="ops-kv"><span class="ops-chip">' + esc(label(c.category)) + '</span><span class="ops-chip">' + esc(label(c.urgency)) + '</span><span class="ops-chip">' + esc(label(c.status)) + '</span><span class="ops-chip">' + esc(c.sla_breached ? "SLA overdue" : "Response due " + due) + '</span></div></div>' + controls + '</article>';
  ...
  function renderCompany(c) {
    ...
    const branches=(c.branches||[]);const branchRows=branches.length?branches.map(function(b){return '<div class="ops-row compact-row"><div class="ops-row-main"><b>'+esc(b.branch_name)+'</b><p>'+esc([b.branch_code,b.city,b.country,b.billing_email].filter(Boolean).join(" / "))+'</p><div class="ops-kv"><span class="ops-chip">'+esc(label(b.status))+'</span></div></div></div>';}).join(''):'<p class="form-note">No branches configured.</p>';const branchForm=canEdit?'<details class="corporate-add-contact"><summary>Add branch</summary><form class="form-grid corporate-branch-form" data-account-id="'+esc(c.id)+'" onsubmit="return false"><div class="field-row"><div class="field col-4"><label>BRANCH NAME</label><input name="branch_name" required></div><div class="field col-2"><label>CODE</label><input name="branch_code"></div><div class="field col-3"><label>CITY</label><input name="city"></div><div class="field col-3"><label>COUNTRY</label><input name="country" value="United Arab Emirates"></div><div class="field col-4"><label>ADDRESS</label><input name="address"></div><div class="field col-3"><label>PHONE</label><input name="phone"></div><div class="field col-3"><label>BILLING EMAIL</label><input name="billing_email" type="email"></div><div class="field col-2"><label>STATUS</label><select name="status"><option value="active">Active</option><option value="on_hold">On hold</option><option value="inactive">Inactive</option></select></div><div class="field col-12"><label>NOTES</label><input name="notes"></div></div><button class="btn btn-primary" type="submit">Save branch</button></form></details>':'';
    ...
    const tone = health.tone === "info" ? "ok" : health.tone;
    return '<details class="corporate-card corporate-' + esc(tone) + '"><summary><div class="ops-row-main"><div class="corporate-company-line"><b>' + esc(c.company_name) + '</b><span>' + esc(label(c.status)) + '</span></div><p>' + esc(c.billing_email || c.accounts_email || "No billing email") + ' / ' + esc(c.phone || "No phone") + '</p><div class="ops-kv"><span class="staff-risk ' + esc(tone === "ok" ? "warn" : tone) + '">' + esc(health.label) + '</span><span class="ops-chip">Terms: ' + esc(label(c.payment_terms)) + '</span><span class="ops-chip">LPO: ' + esc(bool(c.lpo_required)) + '</span><span class="ops-chip">Branches: ' + esc(branches.length) + '</span><span class="ops-chip">Contacts: ' + esc(contacts.length) + '</span><span class="ops-chip">Portal: ' + esc(portalMembers.length) + '</span><span class="ops-chip">Bookings: ' + esc(c.booking_count || 0) + '</span></div></div><div class="corporate-row-value"><span>Account value</span><b>' + esc(money(c.booking_value, "AED")) + '</b></div></summary><div class="corporate-card-body"><div class="corporate-health-strip corporate-health-' + esc(tone) + '"><div><b>' + esc(health.label) + '</b><p>' + esc(health.action) + '</p></div><div class="ops-kv">' + issueChips + '</div></div><div class="ops-grid ops-grid-2"><div><h3>Account controls</h3><div class="ops-kv"><span class="ops-chip">Credit: ' + esc(bool(c.credit_allowed)) + '</span><span class="ops-chip">Monthly billing: ' + esc(bool(c.monthly_billing)) + '</span><span class="ops-chip">Billing email: ' + esc(bool(hasBillingEmail(c))) + '</span><span class="ops-chip">Authorized contact: ' + esc(bool(hasAuthorizedContact(c))) + '</span><span class="ops-chip">Accounts contact: ' + esc(bool(hasAccountsContact(c))) + '</span>' + (c.trade_license_no ? '<span class="ops-chip">TL: ' + esc(c.trade_license_no) + '</span>' : '') + (c.trn ? '<span class="ops-chip">TRN: ' + esc(c.trn) + '</span>' : '') + '</div><p class="form-note">' + esc(c.notes || "No account notes.") + '</p></div><div><h3>Contacts</h3>' + contactRows + '</div></div><section><h3>Branches</h3>'+branchRows+branchForm+'</section>' + portalAccess + statementPanel + form + '</div></details>';
  }
